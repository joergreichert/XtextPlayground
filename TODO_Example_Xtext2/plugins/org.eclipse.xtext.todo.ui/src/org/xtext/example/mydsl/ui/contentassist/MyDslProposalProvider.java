...
public class MyDslProposalProvider extends AbstractMyDslProposalProvider {
	...
	protected Function<IEObjectDescription, ICompletionProposal> getProposalFactory(
		...
		return new org.eclipse.xtext.ui.editor.contentassist.AbstractJavaBasedContentProposalProvider.DefaultProposalCreator(
				contentAssistContext, ruleName, getQualifiedNameConverter()) {
			...
			public ICompletionProposal apply(IEObjectDescription candidate) {
				...
				String proposal = getQualifiedNameConverter().toString(
						candidate.getName());
				if (ruleName != null) {
					try {
						proposal = getValueConverter().toString(proposal,
								ruleName);
				...
				result = createCompletionProposal(proposal, displayString,
						image, contentAssistContext);
				if (result instanceof ConfigurableCompletionProposal) {
					((ConfigurableCompletionProposal) result)
							.setAdditionalProposalInfo(objectOrProxy);
	...
	@Override
	public void completeImport_ImportedNamespace(EObject model,
		...
		if (project != null) {
			List<String> alreadyImported = packageSelector
					.getAlreadyImported(container);
			ePackages.addAll(packageSelector.getEPackages());
			createImportProposals(context, acceptor, project, alreadyImported,
					new ArrayList<EPackage>(ePackages));
		}
	}

	private void createImportProposals(final ContentAssistContext context,
			ICompletionProposalAcceptor acceptor, IJavaProject javaProject,
			List<String> alreadyImported, List<EPackage> ePackages) {
		List<EPackage> filteredEPackages = packageSelector
				.filterAccessibleEPackages(javaProject, ePackages);
	...
	private void createImportProposals(final ContentAssistContext context,
			ICompletionProposalAcceptor acceptor, List<String> alreadyImported,
			EPackage ePackage) {
		...
		if (!alreadyImported.contains(name)) {
			ConfigurableCompletionProposal proposal = (ConfigurableCompletionProposal) createCompletionProposal(
					name, context);
			String displayString = ePackage.getName() + " ("
					+ ePackage.getNsURI() + ")";
